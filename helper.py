...
def calc_accuracy(prediction: torch.Tensor, label: torch.Tensor) -> torch.Tensor:
    hard_prediction = torch.round(prediction)
    return torch.mean((label == hard_prediction).float())
